...
var allSpan1 = {};
var nsmeSpan = ['mark', 'rating', 'first_name', 'last_name', 'patronymic', 'phone', 'vk_link', 'birthdate', 'personalEmoji', 'comment'];
...
$(document).on('click', '#userTable tr', (evn) => {
    ...
    for(var key in allSpan){
        oldData+=allSpan[key].innerText = user[key];
    }
...
$(saveButton).click((evn) => {
    ...
    var change = false
    for(var key in allSpan){
        if(allSpan[key].innerText != selectUser.info[key]){
    ...
    users_info.forEach((user, index) => {
        ...
        html+=`
            <tr data-userIndex='${index}' id="telegramId${user.id}">
                ...
                <td class="info"><span><img src="svg/vk.svg">Вконтакт</span><span><a href="https://vk.com/${user.vk_link.toString().substring(user.vk_link.toString().lastIndexOf('/')+1)}" target="blank">${user.vk_link.toString().substring(user.vk_link.toString().lastIndexOf('/')+1)}</a></span></td>
                <td class="info"><span><img src="svg/telephone.svg">Телефон</span><span><a href="tel:+${user.phone}">${user.phone}</a></span></td>
                <td class="info" ${document.getElementById('style').value}><span><img src="svg/grade.svg">Оценка</span><span>${user.mark} (${user.mark==1?'Вратарь':user.mark==2?'Очень плохо':user.mark==3?'Плохо':user.mark==4?'Хорошо':user.mark==5?'Очень хорошо':user.mark==6?'Отлично':'Новичок'})</span></td>
                <td class="info"><span><img src="svg/award.svg">Рейтинг</span><span>${user.rating} (Баллов)</span></td>
                <td class="info"><span><img src="svg/verification.svg">Проверка</span><span>${user.registrar_type==0?'Не подавал заявки':user.registrar_type==1?'Проверка модераторами':user.registrar_type==2?'Проходит тест':'Данные подтверждены'}</span></td>
                <td class="info"><span><img src="svg/birthday-cake.svg">День рождения</span><span>${user.birthdate}</span></td>
